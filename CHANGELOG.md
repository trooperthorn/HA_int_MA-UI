...
- **Adaptive web player**: the browser player watches its own stream (resyncs, sync error) and steps down a ladder when a link is not keeping up (your settings, then Opus 128 kb/s with a 2.5 s buffer, Opus 96 kb/s with 5 s, Opus 64 kb/s with 10 s), and back up after ten quiet minutes. Codec and bitrate change on the server in place through the player's preferred-format setting and the app's new `sendspin_opus_bitrate` setting; the buffer changes on the running player. Automatic on remote links; the phone menu has the switch, the rung, and an Opus bitrate choice.
- **Duplicates page** (Settings, User Interface, Duplicates): scans the whole library and groups what it holds more than once: one track with several file copies, byte-identical files under different tracks, and probable pairs the import did not merge (same name and artist, duration within 8 s); plus the CUE sheets whose audio is gone. Copies are ranked by format (lossless, then bit depth and sample rate, then bit rate) and by tag completeness, the keeper is marked, and lesser copies can be removed from the library one at a time or in bulk with a confirmation; probable pairs get a merge action instead. CSV export. With the app's trash commands, lesser copies and orphaned CUE sheets can also be moved into `.music-assistant-trash/` on the same drive (a rename, reversible from the page's trash section until it is emptied). See `docs/DUPLICATES.md`.
- **Folder picker for the Filesystem provider**: its path entry, in the add and reconfigure flows, gains a Browse button that opens the folders under the app's music roots (the music drive the app mounts, the media folder, the share folder) so the path is picked rather than typed; a root that is not present says "not mounted". Needs the app's `browse_path` server edit; without it the button hides and the text field stands alone. See `docs/MUSIC-DRIVE.md`.
- **Web player log**: the phone menu keeps the last 400 things the web player did (the Sendspin SDK's own lines, state changes, page visibility, connection changes, a health sample every 30 s while playing) and copies or shares them as text with a header naming the settings in use, so a stall inside the Home Assistant app can be read without a debugger.
- **Web player on Android and Chrome**: the browser player decodes FLAC through the browser's own WebCodecs decoder (as it already did for Opus) instead of rebuilding a FLAC file for every chunk on the main thread; on a phone the audio output asks for the power-friendly `playback` buffer rather than the smallest one; and adaptive mode reads the browser's connection type (data saver, 2G, 3G, a slow round trip) to start on a lower rung and to step down at once when the phone changes networks, before the first dropout. The first two are a build-time patch of `@sendspin/sendspin-js` (`patches/`), see `docs/SENDSPIN-JS-PATCH.md`.
- **Web player over a remote link**: the browser player starts with a 2.5 s buffer and a 1 s startup lead when it is not on the local network (the Home Assistant app away from home comes in through the ingress proxy), instead of the LAN's 0.5 s and 250 ms. On a phone the header's menu icon (in place of the plus, which is now the menu's _Create_ entry) shows what the player negotiated and lets you pick the buffer (0.5 s to 10 s) and the codec (Opus, FLAC, PCM), kept per device.
- **Library source on load**: the source chosen for the Library node (its right-click radio) now narrows the listing the page opens on; before, it only applied once the Library node was clicked, so the first play after a reload could come from every source.
